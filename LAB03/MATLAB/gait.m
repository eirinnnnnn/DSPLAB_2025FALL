...
%         if (wz(n) < Omega_low) && (ay_g(n) < A_low)
%             state = true;
%         end
%     else
%         % Exit stance when EITHER becomes large
%         if (wz(n) > Omega_high) || (ay_g(n) > A_high)
%             state = false;
%         end
%     end
%     is_stance(n) = state;
...
% %% --------------- Integrate with per-step drift removal ---------------
% axE = accE(:,1); ayE = accE(:,2); azE = accE(:,3);
% 
% vx_raw = cumtrapz(t, axE);  vy_raw = cumtrapz(t, ayE);  vz_raw = cumtrapz(t, azE);
% vx = zeros(N,1); vy = zeros(N,1); vz = zeros(N,1);
% sx = zeros(N,1); sy = zeros(N,1); sz = zeros(N,1);
% 
% step_windows = cell(num_steps,1);
% step_length  = zeros(num_steps,1);
% step_height  = zeros(num_steps,1);

%% --------------- Integrate with per-step drift removal (minimal fixes) ---------------
axE = accE(:,1); ayE = accE(:,2); azE = accE(:,3);
...
for s = 1:num_steps
    ...
    % (1) Remove simple DC bias in accel using the previous stance mean
    si = st_starts(s):st_stops(s);     % stance s indices
    ax_bias = mean(axE(si));           % use only X for "straight walk" minimal fix
    ay_bias = mean(ayE(si));
    az_bias = mean(azE(si));
    ...
    % Force velocity to zero at step ends (ZUPT-V) with linear de-drift
    ...
    % (2) Rectify small negative lobes in forward velocity to avoid backtracking
    vx_pos = max(vx_corr, 0);

    % Save velocities for plotting
    vx(rr) = vx_corr; vy(rr) = vy_corr; vz(rr) = vz_corr;
    vx(i1)=0; vx(i2)=0; vy(i1)=0; vy(i2)=0; vz(i1)=0; vz(i2)=0;

    % % Vel -> Disp. For X use rectified velocity; Y,Z use signed velocity
    % sx(rr) = sx(max(i1-1,1)) + cumtrapz(t(rr), vx_pos);
    % sy(rr) = sy(max(i1-1,1)) + cumtrapz(t(rr), vy_corr);
    % sz(rr) = sz(max(i1-1,1)) + cumtrapz(t(rr), vz_corr);
    % ---- direct velocity integration (local time, cumulative base)
tloc = t(rr) - t(rr(1));   % start each segment at 0s

sx(rr) = x_base + cumtrapz(tloc, vx_pos);   % X uses rectified v
sy(rr) = y_base + cumtrapz(tloc, vy_corr);  % Y uses signed v
sz(rr) = z_base + cumtrapz(tloc, vz_corr);  % Z uses signed v

% Update cumulative bases at the end of this step
x_base = sx(rr(end));
y_base = sy(rr(end));
z_base = sz(rr(end));

% (nice to have) hold the value constant through the following stance
if s+1 <= num_steps
    ss = st_starts(s+1):st_stops(s+1);
    sx(ss) = x_base; sy(ss) = y_base; sz(ss) = z_base;
end
...
% for s=1:num_steps
%     i1 = st_stops(s);         % end of stance s (start of swing)
%     i2 = st_starts(s+1);      % start of next stance
%     if i2 <= i1, continue; end
%     rr = i1:i2;
% 
%     % Linear drift between endpoints
%     vx_d = linspace(vx_raw(i1), vx_raw(i2), numel(rr))';
%     vy_d = linspace(vy_raw(i1), vy_raw(i2), numel(rr))';
%     vz_d = linspace(vz_raw(i1), vz_raw(i2), numel(rr))';
% 
%     vx(rr) = vx_raw(rr) - vx_d;
%     vy(rr) = vy_raw(rr) - vy_d;
%     vz(rr) = vz_raw(rr) - vz_d;
% 
%     % Zero velocity at stance boundaries
%     vx(i1) = 0; vx(i2) = 0;
%     vy(i1) = 0; vy(i2) = 0;
%     vz(i1) = 0; vz(i2) = 0;
% 
%     % Displacements within step (accumulate from previous)
%     sx(rr) = sx(max(i1-1,1)) + cumtrapz(t(rr), vx(rr));
%     sy(rr) = sy(max(i1-1,1)) + cumtrapz(t(rr), vy(rr));
%     sz(rr) = sz(max(i1-1,1)) + cumtrapz(t(rr), vz(rr));
% 
%     step_windows{s} = [t(i1), t(i2)];
%     step_length(s)  = sx(i2) - sx(i1);
%     step_height(s)  = max(sy(rr)) - min(sy(rr));
% end

% Outside steps: keep last positions, zero velocities
mask = false(N,1);
for s=1:num_steps
...
sx = fill_forward_const(sx, mask);
sy = fill_forward_const(sy, mask);
sz = fill_forward_const(sz, mask);
